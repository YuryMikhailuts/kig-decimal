...
class BigDecimalTest {
	...
	fun divideTest() {
		for (l in -100..100) {
			for (r in -100..100) {
				...
				val actual = l.toBigDecimal().divide(r.toBigDecimal(), MathContexts.DECIMAL128)
				// TODO: Это очень плохо, но без doubleValue() лучше не сделать.
				assertTrue { "${expect.subtract(actual)}".toDouble() < 0.01 }
			}
		}
	}
	...
	fun intValueExactTest() {
		for (r in 1..100) {
			assertEquals(r, r.toBigDecimal().intValueExact())
		}
	}
